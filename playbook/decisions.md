...
- **`/outreach` pushes every drafted email into Zoho Mail as a real draft
  (Stage 7.5, `tools/zoho-draft-push/`), never sends.** Zoho saves a draft
  and sends an email through the *same* endpoint under the same
  `ZohoMail.messages.CREATE` scope, distinguished only by the request body's
  `mode` field — so the scope is not what prevents a send. What prevents it
  is the tool building its payload as a closed set of literal keys with
  `mode` hardcoded to `"draft"`, never derived from campaign data, with a
  test asserting that on every payload. Reply, delete and folder-move are
  separate API surfaces the code never calls. Credentials live at
  `~/.wardith/zoho-credentials.json`, outside the repo, generated by the
  owner directly in Zoho's own API console. 2026-08-24, corrected
