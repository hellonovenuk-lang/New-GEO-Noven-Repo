...
Ingest the campaign into the CRM before this step, then push only the
...
Run this before Stage 7.5 for a new campaign, so draft creation can reconcile
against its CRM record; run it again afterwards only if the saved prep output
needs refreshing:
