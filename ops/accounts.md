...
| **Zoho Mail Lite** | `hello@novenstudio.co.uk` — the licensed user, and the only inbound channel on a site with no phone and no form. **`hello@wardith.co.uk` has not been created yet**; when it is, it goes on this same licence as a free alias, never as a second user (£1/month each). The old address must then keep receiving for at least twelve months: it is published in the ICO record, on both LinkedIn pages and in whatever has already been cached | £14.40/yr inc VAT | ~29 Jul 2027 *(inferred from purchase date, not recorded)* | No enquiry reaches the business |
...
| **Google Search Console** | Sitemap, indexation, removals. **The verified property is `novenstudio.co.uk`; `wardith.co.uk` has none yet** — a property is bound to the host it was verified for, so the rename did not carry it. Keep the old property permanently: it holds the before/after the six-month re-check needs | Free | n/a | The only indexation diagnostic |
