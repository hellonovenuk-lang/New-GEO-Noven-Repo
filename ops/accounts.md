...
| **UK Postbox** *(chosen 2026-08-07, **not yet bought** — fill this row in on the day)* | **The address for service of documents**: the legal disclosure in the site footer, the ICO registered address, and the trading-name registration for "Wardith". Company 06723381, HMRC AML XLML00000192390 | **£12/month inc VAT** (£10 exc). PAYG page scans £1.20 each. `[PLACEHOLDER: setup fee, if any — read the checkout total]` | Monthly, rolling. `[PLACEHOLDER: billing date]` | **A failed £12 card payment silently breaks a legal disclosure.** Their terms hold mail for one month on an inactive account, then **return it to sender**; at six months the account is purged and post is shredded. And the address cannot simply be dropped — it must be migrated off ICO, HMRC and the site *first*, or continued use stays chargeable. `ops/third-party-services.md` B1c |
...
| Dependency | Status |
|---|---|
| **Address for service** | **Chosen by the owner 2026-08-07: UK Postbox Business Street Address, Poole, £12/month inc VAT** — monthly, on the owner's cash-flow constraint, which ruled out the better-reviewed annual-only providers. Runbook at `ops/third-party-services.md` **B1c**. **Not yet ordered.** Move the row below into the register above on the day it goes live. **V LOT written off the same day — refund requested**; its order reference, amount and payment method are **all still unrecorded**, which is what a chargeback needs if the refund is refused |
