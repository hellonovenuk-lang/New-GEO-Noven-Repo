...
|---|---|---|---|---|---|
| 1 | The **nine** site pages | Repo → Netlify | Yes | **Verified 2026-08-06 against the built output of the live commit.** `main` is at `d35557a`, deployed to production and `ready` per the Netlify API, and the `dist` for that commit carries 250 / 800 / 150 / 400 / 700, every canonical on `wardith.co.uk`, no `[PLACEHOLDER]` anywhere, and the homepage's visible code block byte-for-byte identical to its head JSON-LD. **The live-host gap is now closed too, 2026-08-07:** the owner ran Google's URL Inspection on the homepage and supplied the returned HTML — zero `PLACEHOLDER`, the `company/wardith` `sameAs` correctly formed, `hello@wardith.co.uk` in both the Organization and the `contactPoint`, the canonical right, and no "Noven" anywhere. **That is the first time anything in this repo has been checked against what a crawler actually receives** rather than against `dist` or the Netlify API | Owner: eyeball the live pricing page once, which is the only page carrying prices that has not been seen live. The homepage no longer needs it |
...
| 3 | **LinkedIn company page — About** | Owner, in LinkedIn | Yes | **Fixed 2026-08-06.** Rewritten by the owner: no Noven, prices matching the site, and `hello@wardith.co.uk` — confirmed after the mailbox moved later the same day | Re-check at the next sweep, not before |
| 4 | **LinkedIn founder profile — About** | Owner, in LinkedIn | Yes | **Fixed 2026-08-06**, same pass, and the contact address re-checked after the mailbox moved | Re-check at the next sweep |
...
| 7 | Google Search Console | Owner | Indirect | **Complete 2026-08-06.** `wardith.co.uk` verified as a Domain property via GoDaddy Domain Connect; sitemap submitted; indexing requested on all eight pages; **Change of Address accepted** and running to roughly Feb 2027. **The `novenstudio.co.uk` property is kept permanently** — the move runs *from* it, and it holds half the six-month measurement | Nothing. Re-run `site:novenstudio.co.uk` at one month and six — baseline **4** |
| 8 | Bing Webmaster Tools | Owner | Indirect | **Set up 2026-08-07.** `wardith.co.uk` submitted and indexing requested on all eight indexable pages. Nothing was migrated because there was nothing to migrate — Bing had never indexed the old domain. **One thing is recorded as unconfirmed rather than assumed:** whether the sitemap is listed under Bing's Sitemaps panel. URL Submission does not require it, so doing the former does not prove the latter. **Confirmed done 2026-08-07** — the sitemap is in. **`site:wardith.co.uk` on Bing returns nothing as at 2026-08-07**, which is the expected reading one day after submission on a domain four days old with no inbound links, and is the zero this measurement starts from | Owner: re-run `site:wardith.co.uk` on Bing weekly. **Eight pages returned is what closes self-audit finding 2** — nothing before that does |
| 9 | ICO public register entry | ICO | No | **Amended 2026-08-10**: the owner notified the ICO of both the address change — off the home address, onto `Lytchett House, Poole` — and the trading-name change to Wardith. Whatever published before the amendment is permanent and mirrored, and cannot be checked retrospectively | Owner: look up `C1995412` on the ICO's public register in a browser (it 403s automated fetches) and confirm it shows **Wardith** at **Lytchett House**. Both halves. This is the last open item on the address |
| 10 | Email signature on `hello@` | Owner, in Zoho | **No.** Answering the old placeholder: it carries no price, and by design — a price in a signature is a fact on the highest-frequency surface the business has, changing every time the ladder moves | **Replaced 2026-08-07** with `assets/brand/email-signature.html`. The supplied `Email Signature.svg` said `hello@wardith.com` above `wardith.co.uk` and was never installed. **The new one is live text, not outlined paths**, so a fact change is an edit rather than a Canva round trip — which is what caused the two-domain fault in the first place | Owner: install in Zoho for new mail **and** replies. **The address-for-service row is filled in and uncommented as of 2026-08-10**, so the file has changed since it was last pasted — reinstall it rather than assuming the live signature matches |
...
| 15 | **The address for service**, on every surface at once — site footer (nine pages), the `PostalAddress` in the Organization JSON-LD, `/terms/`, the ICO register, the Zoho email signature, and every outreach email | Repo → Netlify for the first three; the owner everywhere else | No | **New and published 2026-08-10.** One value in `site/src/data/business.ts` drives the three site surfaces, so they cannot drift from each other. **The other three are copies and can.** UK Postbox also issued a courier-only address on a different postcode (`BH16 6FH`); it is published nowhere and must stay that way | Owner: this row is the reason a change of address would be expensive. If it ever changes, the copies are the risk — Zoho, the ICO, and anything already sent. Check the signature and the ICO entry match the repo at each sweep |
