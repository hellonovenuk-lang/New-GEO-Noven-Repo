...
| 8 | Bing Webmaster Tools | Owner | Indirect | **Set up 2026-08-07.** `wardith.co.uk` submitted and indexing requested on all eight indexable pages. Nothing was migrated because there was nothing to migrate — Bing had never indexed the old domain. **One thing is recorded as unconfirmed rather than assumed:** whether the sitemap is listed under Bing's Sitemaps panel. URL Submission does not require it, so doing the former does not prove the latter, and the import from Search Console is the only step that would have carried it across silently | Owner: open **Sitemaps** in Bing once and confirm `sitemap-index.xml` is listed; submit it if not. Then `site:wardith.co.uk` on Bing weekly until eight pages return |
