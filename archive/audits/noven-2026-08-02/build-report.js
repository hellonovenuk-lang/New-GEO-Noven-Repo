...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      h2('What we found on your website'),
      body('We check four things, in this order. Most of them your site already gets right — worth saying plainly, because a report that lists only faults can’t tell you whether your site is in good order or whether we simply didn’t look.'),
      body('**1. Can the assistants get to you at all? Three of the four, yes. Copilot, no.** Your site invites every major assistant’s software by name and turns none of them away. There is a working list of your pages for them to follow and it points at the right address. No password, no redirect and no security challenge stands in front of any of it — which is where we most often find a business accidentally shut out. The exception is Microsoft: it holds no record of your site at all, so Copilot cannot reach you however open the door is. That’s finding 2 below.'),
      body('**2. Can they read you once they’re in? Yes.** Your words are in the page itself rather than assembled afterwards inside a visitor’s browser. That sounds like a detail and isn’t: sites built with some popular tools arrive as an empty shell that only a person’s browser fills in, and an assistant reading one of those gets nothing at all. Yours is readable as it stands, on every page.'),
      body('**3. Are your business facts written where a machine can find them? Nearly all of them.** Your name, what you do, your email, your services and all five of your prices exist in a form built for machines, not only as words on a page. They are produced from a single record, so what a person reads and what a machine reads cannot drift apart — a problem we find on most sites and won’t find on yours. **Publishing your prices is the most valuable single thing on this list**, and most businesses don’t: an assistant asked “roughly what should this cost” can only name a business that published a number.'),
      body('One fact is missing from that record, and it’s the one you’d most want there: **where you work.** The pages say “the Wirral” in words a person reads. The machine-readable version says only “United Kingdom” — no town, no area. That’s finding 3.'),
      body('**4. Does anything on your site answer the questions customers ask? Six of the ten.** Your pricing page answers “what are my options and what should it cost” outright, with real figures, which is the hardest of the ten to answer well. Two have nothing behind them at all: **someone looking on the Wirral**, and **who your alternatives are**. Those two are the gap list — and the Wirral one is the same problem as the missing location, showing up from a different direction.'),
      body('So: two of the four in good order, one missing a single fact, one absent from Microsoft’s records altogether. A better starting position than most sites we see — which is why the recommendation below is short.'),
      ...
      body('**2. Copilot doesn’t have your website on file at all.** It cost you every one of the nine hand-checked Copilot answers, and it would stay true however good your website was — your site invites Microsoft in, but nobody has told Microsoft it exists. Registering is free and takes about fifteen minutes, then days or weeks before anything shifts.'),
      body('**3. Nothing tells a machine where you work.** It cost you all 15 checks when we asked for someone on the Wirral: there was simply nothing to match you against. This is the one item here that is a small, contained change to your website, and fixing it closes one of the two missing question pages at the same time.'),

      h2('What we’d do about it'),
      body('Findings 2 and 3 are ordinary Foundation work, both worth doing. Finding 1 isn’t, and it’s the bigger problem: no amount of website work stops an assistant thinking you’re a pharmaceutical company, and every improvement under the current name accrues to a name four other businesses answer to.'),
